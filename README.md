...
2. **Выберите архитектуру и запустите:**
   
   **Для продакшена (6+ сервисов) - Domain-specific HA OTEL:**
   ```bash
   task up-all  # Domain-specific HA (рекомендуется)
   ```
   
   **Для разработки (<6 сервисов) - Unified HA OTEL:**
   ```bash
   task up-all-unified  # Единый HA коллектор
   ```
   
   **Для отладки - Simple OTEL:**
   ```bash
   task up-all-simple  # Простой коллектор
   ```
...
- **API Gateway (Envoy)**: http://localhost:8080 - единая точка входа для всех API
- **Envoy Admin**: http://localhost:8081 - административный интерфейс Envoy

### Domain-specific OTEL HAProxy Stats:
- **Logs OTEL HAProxy**: http://localhost:8407/stats - статистика logs коллекторов
- **Traces OTEL HAProxy**: http://localhost:8408/stats - статистика traces коллекторов  
- **Metrics OTEL HAProxy**: http://localhost:8409/stats - статистика metrics коллекторов
...
- `envoy` - API Gateway с gRPC-JSON транскодированием
...
- `POST /api/v1/external-auth/*` - Внешние провайдеры аутентификации
- `GET /healthz` - Проверка здоровья системы

### Защищенные эндпоинты (требуют авторизации):
- `GET /api/v1/users` - Управление пользователями
- `GET /api/v1/whoami` - Информация о текущем пользователе
- `GET /api/v1/roles` - Управление ролями
- `GET /api/v1/permissions` - Управление разрешениями
- `GET /api/v1/user-roles` - Назначение ролей пользователям
- `GET /api/v1/role-permissions` - Назначение разрешений ролям
...
- Session-based аутентификация через Envoy External Authorization
...
- CORS настройки для безопасного доступа из браузера
