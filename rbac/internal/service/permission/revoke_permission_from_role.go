...
func (s *PermissionService) RevokePermissionFromRole(ctx context.Context, roleID, permissionID string) error {
	ctx, span := tracing.StartSpan(ctx, "rbac.service.revoke_permission_from_role")
	defer span.End()

	err := s.rolePermissionRepo.RevokePermissionFromRole(ctx, roleID, permissionID)
	if err != nil {
